...
- **Function name**: `IssueTracker-APIAuthorizer`
- **Runtime**: `Python 3.11`
- **Architecture**: `x86_64`
- **Execution role**: `Use an existing role` → `IssueTracker-Lambda-ExecutionRole`
...
2. Add these variables:
   ...
   - **Key**: `COMPANIES_TABLE_NAME` **Value**: `IssueTracker-CompanyManagement`
...
- **Type**: `Lambda`
- **Lambda Function**: `IssueTracker-APIAuthorizer`
...
- **Table name**: `IssueTracker-RateLimits`
...
def check_and_update_rate_limit(company_id, subscription_tier):
    ...
    try:
        ...
        # Get current usage
        response = dynamodb.Table('IssueTracker-RateLimits').get_item(
            Key={
                'company_id': company_id,
        ...
        # Increment counter
        dynamodb.Table('IssueTracker-RateLimits').update_item(
            Key={
                'company_id': company_id,
                'time_window': current_hour
            },
...
- **Rate Limit Table**: `IssueTracker-RateLimits`
...
_💡 Pro Tip: Save your JWT tokens and User Pool details - you'll need them for testing and frontend integration!_
